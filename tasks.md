...
- [x] **Upsell Block Above-the-Fold Fix — All Resolutions (`assets/custom-upsells.css`):** Root cause: `.upsell_block` had no `max-height`, so tall content could exceed the viewport height with no internal scroll. On mobile, `.upsell_overlay` used `margin-top: 70px; height: 100%` on a `position: fixed` element — `height: 100%` = 100vh, so the overlay extended 70px below the visible viewport bottom, and `overflow-y: auto` scrolled the overlay itself rather than the block. Fixes: (1) Added `max-height: calc(100vh - 40px); overflow-y: auto` to global `.upsell_block` — block is always bounded by viewport at desktop and tablet, scrolls internally if needed. (2) On `max-width: 767px`: changed overlay from `margin-top: 70px; height: 100%` → `top: 70px; height: calc(100vh - 70px)` so it fills exactly the space below the header with no off-screen overflow; `overflow-y: hidden` on overlay (scroll is on the block). Block gets `max-height: calc(100vh - 90px); overflow-y: auto` — fits in the remaining space with 20px breathing room. Also removed 5 dead CSS comment blocks from the file.
  - Speed Win: Upsell modal is fully visible on first render at all viewport sizes; no page or overlay scroll required to see View Cart button.
