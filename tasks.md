...
- [x] **Homepage Product Card Image Overflow Fix at ≥768px (`assets/main.css`, `assets/theme.css`):** Root cause: `.main__products__item-img { max-width: 260px }` used an absolute pixel value matching the item's content-box width exactly — images at or near their natural 260px width had no room for rounding/intrinsic-size variance and could overflow. Fix: changed to `max-width: 100%; height: auto; display: block; margin: 0 auto` — image is now always bounded by its parent content area (260px), auto-height preserves aspect ratio, `display: block` removes descender gap, `margin: 0 auto` centers smaller images. Removed now-redundant `@media (max-width: 767px) .main__products__item-img { max-width: 100% }` from `theme.css` (identical value, dead rule after main.css change).
  - Speed Win: Images cleanly contained and centered; one dead CSS declaration removed.
