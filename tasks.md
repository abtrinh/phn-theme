...
- [x] **Yotpo Reviews Widget — Mobile Layout Fix (`assets/patches_productpatch.css`):** Yotpo's injected fixed-width CSS was causing horizontal overflow and scroll on mobile. Added `@media (max-width: 768px)` overrides: `overflow-x: hidden` on commentlist wrapper + Yotpo root; `max-width: 100%; box-sizing: border-box` on all `.yotpo *` elements; `flex-direction: column; align-items: center` on bottom-line and filters panel to stack vertically; `white-space: nowrap; font-size: 30px` on `.yotpo-headline` to keep "Customer Reviews" single-line; "Write A Review" button centered via `display: block; margin: 0 auto; float: none`.
    - Speed Win: Eliminated horizontal overflow in reviews section; all elements now stack and center correctly on mobile without horizontal scroll.
