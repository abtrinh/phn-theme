...
- [ ] **Related Products Not Clickable — calm-relax product page:** User reports related product cards are not clickable on the calm-relax page but work on other product pages. Needs investigation of `sections/calm-relax.liquid` for overlapping elements, `pointer-events: none`, z-index stacking, or position/overflow CSS blocking click events on the `product-recommendations` section.
